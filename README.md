...
**Last confirmed working: 2026-09-11** on Arch Linux, KDE Plasma 6.7.5
...
there. Non-Arch distributions are not supported at all, since the script is
built around `pacman`.
...
standalone, and never assumes a local copy of those files.
...
It only downloads the `loose/` payload when step 9 or step 11 is selected.
